...
class Interpreter:
    ...
    def __init__(
        ...
        **kws,
        ...
        self.max_statement_length = max(1, min(1.0e8, max_statement_length))

        self.use_numpy = HAS_NUMPY and use_numpy
        if symtable is None:
            symtable = make_symbol_table(
                nested=nested_symtable,
    ...
    def raise_exception(self, node, exc=None, msg="", expr=None, lineno=None):
        ...
        err = ExceptionHolder(node, exc=exc, msg=msg, expr=self.expr, text=text, lineno=lineno)
    ...
    def on_dict(self, node):  # ('keys', 'values')
        """Dictionary."""
        return {self.run(k): self.run(v) for k, v in zip(node.keys, node.values)}
    ...
    def on_slice(self, node):  # ():('lower', 'upper', 'step')
        """Simple slice."""
        return slice(self.run(node.lower), self.run(node.upper), self.run(node.step))
        ...
        for tnode in node.targets:
            ...
            if tnode.__class__ == ast.Name and tnode.id not in self.readonly_symbols:
                ...
                while tnode.__class__ == ast.Attribute:
                    children.append(tnode.attr)
                    tnode = tnode.value
                if tnode.__class__ == ast.Name and not tnode.id in self.readonly_symbols:
                    children.append(tnode.id)
                    children.reverse()
                    sname = ".".join(children)
                    val = self.run(sname)
    ...
    def _comp_save_syms(self, node):
        ...
        for tnode in node.generators:
            if tnode.target.__class__ == ast.Name:
                if (
                    not valid_symbol_name(tnode.target.id)
                    or tnode.target.id in self.readonly_symbols
                ):
    ...
    def do_generator(self, gnodes, node, out):
        ...
        if gnode.target.__class__ == ast.Name:
            if not valid_symbol_name(gnode.target.id) or gnode.target.id in self.readonly_symbols:
    ...
    def on_functiondef(self, node):
        ...
        if isinstance(varkws, ast.arg):
            varkws = varkws.arg
        self.symtable[node.name] = Procedure(
            node.name,
            self,
            doc=doc,
            lineno=self.lineno,
