...
// If you want to keep this shape for docs, prefix to silence unused var rule
// interface _EvoResponse {
//   conflicts: Conflict[];
// }
...
const EvoDashboard = () => {
  ...
  const renderContent = () => {
    ...
    if (!conflicts || conflicts.length === 0)
      return <p style={{ color: theme.colors.muted }}>No open conflicts found.</p>;
      ...
      >
        ...
        <span
          style={{
            display: 'block',
            fontSize: '12px',
            color: c.severity === 'high' || c.severity === 'critical' ? '#f87171' : '#facc15',
  ...
  return (
    <Card title="Conflict Center (Live)">
      <p style={{ margin: '0 0 16px', color: theme.colors.muted }}>
        Live queue of open conflicts detected within the system. Click an item to trace its decision.
      </p>
