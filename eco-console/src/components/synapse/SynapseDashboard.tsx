// src/components/synapse/SynapseDashboard.tsx (ingest-path: uses only mounted endpoints)
...
const SynapseDashboard: React.FC = () => {
  ...
  return (
    <div style={{ display: 'grid', gap: '24px', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))' }}>
      ...
      <Card title="Alignment Preference Voting (Arm vs Arm)">
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          <select value={pickA} onChange={e => setPickA(e.target.value)} style={{ ...selectStyle, minWidth: 220 }}>
            ...
            {armOptions.map(id => <option key={`a-${id}`} value={id}>{id}</option>)}
          </select>
          ...
          <select value={pickB} onChange={e => setPickB(e.target.value)} style={{ ...selectStyle, minWidth: 220 }}>
            ...
            {armOptions.map(id => <option key={`b-${id}`} value={id}>{id}</option>)}
          </select>
      ...
      <Card title="Controls">
        ...
        <button onClick={handleReload} style={theme.styles.button}>Reload Arm Registry</button>
      </Card>

      <Card title="Tools Registry Explorer" style={{ gridColumn: '1 / -1' }}>
        <DataTable
          headers={[{ key: 'name', label: 'Tool' }, { key: 'desc', label: 'Description' }]}
          ...
          <div style={{ height: '400px', marginBottom: '24px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={leaderboard} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                ...
                <XAxis dataKey="arm_id" stroke={theme.colors.muted} fontSize={12} interval={0} angle={-30} textAnchor="end" height={80} />
                <YAxis stroke={theme.colors.muted} fontSize={12} />
                <Tooltip contentStyle={{ background: theme.colors.card, borderColor: theme.colors.edge, color: theme.colors.ink }} cursor={{ fill: 'rgba(255,255,255,0.05)' }} />
...
function normalizeLeaderboard(raw: any): LeaderboardRow[] {
  ...
  if (raw && (Array.isArray(raw.top) || Array.isArray(raw.bottom))) {
    const pick = (arr: any[]) => (arr || []).map((r: any) => ({
