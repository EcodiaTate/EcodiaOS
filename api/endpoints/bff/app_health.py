...
def _score(p: str) -> int:
    for i, pat in enumerate(PREF_ORDER):
        if re.search(pat, p): return 100 - i
    return 10
...
def _candidates(entries: List[Dict[str, Any]]) -> List[str]:
    ...
    c = [e["path"] for e in entries if "GET" in e.get("methods", []) and not e.get("has_path_params") and not DANGEROUS.search(e["path"])]
    ...
    for p in _candidates(entries):
        try:
            ...
            if 200 <= code < 300:
                ...
                return {"name": name, "ok": status in ("ok","degraded"), "status": status, "via": p, "latency_ms": round(lat,1)}
        except Exception:
            continue
    return {"name": name, "ok": False, "status": "down", "via": None}

async def _aggregate(required: Optional[List[str]] = None):
    mf = await _manifest()
    ...
    overall = "ok"
    if any(r["status"] == "down" for r in results): overall = "down"
    elif any((r["status"] == "degraded") or (not r["ok"]) for r in results): overall = "degraded"
...
async def app_readyz():
    ...
    return Response(content=("OK" if ready else "NOT_READY"), media_type="text/plain", status_code=(200 if ready else 503))
