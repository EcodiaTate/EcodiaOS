...
from core.utils.net_api import ENDPOINTS, get_http_client  # uses your net_api overlay

Json = Dict[str, object]
CheckKey = str

# ONE place to declare required/optional alias keys per service
...
def _runner(required: Iterable[CheckKey], optional: Iterable[CheckKey] = ()):
    # FIXED: Removed semicolon and fixed indentation for 'opt'
    req = list(required)
    opt = list(optional)
    async def _run():
...
def build_status_health_router(*, service: str, version: str = None,
                      required_keys: Iterable[CheckKey], optional_keys: Iterable[CheckKey] = ()):
    health_router = APIRouter(prefix=f"/{service}", tags=[f"{service}-status"])
    version = version or os.getenv("ECODIAOS_VERSION", "dev")
    runner = _runner(required_keys, optional_keys)

    # FIXED: Removed redundant re-initialization of health_router
    ...
    async def status():
        ...
        # FIXED: Corrected indentation of this if statement
...
def health_router_for(service: str) -> APIRouter:
    cfg = SERVICE_CHECKS[service]
    return build_status_health_router(service=service,
