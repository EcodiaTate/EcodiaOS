...
FROM_NUMPY_FINANCIAL = ("fv", "ipmt", "irr", "mirr", "nper", "npv", "pmt", "ppmt", "pv", "rate")
...
def safe_getattr(obj, attr, raise_exc, node):
    """safe version of getattr"""
    unsafe = attr in UNSAFE_ATTRS or (attr.startswith("__") and attr.endswith("__"))
    if not unsafe:
...
class ExceptionHolder:
    ...
    def __init__(self, node, exc=None, msg="", expr=None, text=None, lineno=None):
        """TODO: docstring in public method."""
        self.node = node
        self.expr = expr
        self.msg = msg
...
class Group(dict):
    ...
    def _repr_html_(self):
        ...
        html = [
            f"<table><caption>Group('{self.__name__}')</caption>",
...
class Procedure:
    ...
    def __setattr__(self, attr, val):
        if not getattr(self, "__ininit__", True):
            self.__raise_exc__(None, exc=TypeError, msg="procedure is read-only")
    ...
    def __call__(self, *args, **kwargs):
        ...
        try:
            ...
            if self.__varkws__ is not None:
                ...
                self.__raise_exc__(None, msg=msg, exc=TypeError, lineno=self.lineno)

        except (ValueError, LookupError, TypeError, NameError, AttributeError):
