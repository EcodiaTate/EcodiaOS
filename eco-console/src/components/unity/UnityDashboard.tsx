/* D:\EcodiaOS\eco-console\src\components\unity\UnityDashboard.tsx */
...
const UnityDashboard = () => {
  ...
  const handleSelect = async (sessionId: string) => {
    ...
    if (selected?.session.id === sessionId) return;
    ...
    <Card title="Deliberation Room">
      <div style={{ display: 'flex', gap: '24px', minHeight: '60vh' }}>
        <div style={{ width: '30%', borderRight: `1px solid ${theme.colors.edge}`, paddingRight: '24px' }}>
          ...
          {isLoadingList ? <p>Loading...</p> : error ? <p style={{color: '#ef4444'}}>{error}</p> : (
            <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
  
              {deliberations.map(d => (
                <li key={d.id} onClick={() => handleSelect(d.id)} style={{
                  padding: '10px', 
                  borderRadius: '6px', 
                  cursor: 'pointer', 
     
                  marginBottom: '8px', 
                  background: selected?.session.id === d.id ? 'rgba(244, 211, 94, .15)' : 'rgba(255,255,255,.05)',
                  border: `1px solid ${selected?.session.id === d.id ? theme.colors.g3 : 'transparent'}`
                  }}>
        ...
        <div style={{ width: '70%' }}>
            {isLoadingDetails ?
 <p>Loading details...</p> : !selected ? <p style={{color: theme.colors.muted}}>Select a deliberation to view its transcript.</p> : (
                <>
                    ...
                    {selected.verdict ? (
                        ...
                        <div style={{...theme.styles.card, background: 'rgba(0,0,0,.2)', marginBottom: '16px'}}>
                          <strong>Verdict: {selected.verdict.outcome}</strong> (Confidence: {(selected.verdict.confidence * 100).toFixed(1)}%)
                          {selected.verdict.dissent && <p style={{margin: '8px 0 0', color: theme.colors.muted, fontSize: '14px', fontStyle: 'italic'}}>{selected.verdict.dissent}</p>}
                    ...
                    <div style={{maxHeight: '50vh', overflowY: 'auto', paddingRight: '10px'}}>
                      {selected.transcript.map((turn) => (
                        ...
                        <div key={turn.turn} style={{ marginBottom: '16px' }}>
                          <strong style={{ fontFamily: theme.fonts.heading, color: theme.colors.g3 }}>{turn.role} (Turn {turn.turn}):</strong>
                          <p style={{ whiteSpace: 'pre-wrap', fontFamily: 'monospace', margin: '4px 0 0', color: theme.colors.muted, fontSize: '14px' }}>{turn.content}</p>
