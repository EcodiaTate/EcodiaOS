...
async def _query_arms_for_mode(mode: str, limit: int = 5000) -> list[dict[str, Any]]:
    # MODIFIED: This function now returns a list of dicts with id and a constructed metadata object.
    def process_rows(rows: list[dict]) -> list[dict[str, Any]]:
        ...
        for r in rows:
            ...
            # Construct the metadata object from individual properties
            metadata = {
                "mode": r.get("mode"),
                "policy_graph": r.get("policy_graph"),
            }
            # Attempt to parse policy_graph if it's a JSON string
    ...
    # The Cypher query is now updated to fetch individual properties
    base_query = """
        MATCH (p:PolicyArm)
        WHERE {where_clause}
    ...
    if mode == "simula_planful":
        where = "p.id STARTS WITH 'simula.planner.' OR p.id STARTS WITH 'simula.agent.'"
        rows = (
    ...
    rows = (
        await cypher_query(
            base_query.format(where_clause="coalesce(p.mode,'') <> ''"), {"limit": int(limit)}
        )
        or []
    )
    return process_rows(rows)
...
async def _ensure_safe_fallback() -> str:
    ...
    try:
        await cypher_query(
            ...
            ON CREATE SET p.mode = 'generic', p.dynamic = true, p.created_ts = timestamp()
            SET p.mode = coalesce(p.mode, 'generic')
            """,
            {"id": arm_id},
        )
    except Exception as e:
        logger.warning("Failed to ensure safe fallback arm: %s", e)
    return arm_id


def _pick_champion_arm(arms: list[dict[str, Any]], mode: str) -> dict[str, Any]:
    if not arms:
        return {"id": "generic.safe_fallback", "metadata": {"mode": "generic"}}
    if mode == "simula_planful":
        for arm in arms:
...
async def _run_llm_planner(
    ...
    try:
        ...
        llm_response = await call_llm_service(
            prompt_response, agent_name="Synapse.Planner", scope=scope
        )
        ...
        logger.info(
            "Episode %s: Re-assigned to dynamic arm '%s' from planner.",
            episode_id,
            dynamic_arm_id,
        )
        ...
        logger.error("Episode %s: LLM Planner failed: %s", episode_id, e, exc_info=True)
...
async def select_or_plan(req: SelectArmRequest) -> SelectArmResponse:
    ...
    if not arms_with_meta:
        chosen_id = await _ensure_safe_fallback()
        chosen_metadata = {"mode": "generic"}
        top_score = 0.0
    else:
        chosen_arm_dict = _pick_champion_arm(arms_with_meta, mode)
        chosen_id = chosen_arm_dict.get("id", "generic.safe_fallback")
    ...
    if task_key in NO_PLANNER_TASK_KEYS:
        return SelectArmResponse(
            episode_id=episode_id, champion_arm=initial_champion_score, shadow_arms=shadow_arms
        )

    if mode == "voxis_planful" and top_score < CONFIDENCE_THRESHOLD:
        ...
        if cached := _planner_cache_get(cache_key):
            ...
            return cached
        resp = await _run_llm_planner(req, episode_id, initial_champion_score, scope, summary)
        _planner_cache_put(cache_key, resp)
        return resp

    if mode == "generic" and top_score < CONFIDENCE_THRESHOLD:
