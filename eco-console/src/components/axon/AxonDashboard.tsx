// src/components/axon/AxonDashboard.tsx
...
const AxonDashboard = () => {
    ...
    const handleSynthesize = useCallback(async () => {
        ...
        try {
            ...
            setNewName('');
            setNewSpecUrl('');
            setTimeout(() => refetch(), 1500); // Use refetch from the hook to refresh data
        } catch (err: any) {
        ...
        <>
            <Card title="Axon Driver & Capability Manager (Live)">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                    ...
                    <button style={theme.styles.button} onClick={() => setIsModalOpen(true)}>Synthesize New Driver</button>
                </div>
                {renderContent()}
            </Card>
            ...
            {isModalOpen && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
                    <Card title="Synthesize New Driver" style={{ minWidth: '500px' }}>
                        <p style={{color: theme.colors.muted, margin: '0 0 16px'}}>Provide an OpenAPI spec URL to automatically generate a new driver.</p>
                        <input type="text" placeholder="Driver Name (e.g., weather_api_driver)" value={newName} onChange={e => setNewName(e.target.value)} style={inputStyle} />
                        <input type="text" placeholder="OpenAPI/Swagger Spec URL" value={newSpecUrl} onChange={e => setNewSpecUrl(e.target.value)} style={inputStyle} />
                        <div style={{ marginTop: '20px', display: 'flex', gap: '12px' }}>
                            <button onClick={handleSynthesize} disabled={isSynthesizing} style={theme.styles.button}>{isSynthesizing ? 'Synthesizing...' : 'Start Synthesis'}</button>
                            <button onClick={() => setIsModalOpen(false)} style={{...theme.styles.button, background: 'transparent', border: `1px solid ${theme.colors.edge}`}}>Cancel</button>
                        </div>
                    </Card>
                </div>
            )}
...
const inputStyle: React.CSSProperties = { width: '100%', padding: '10px', background: 'rgba(0,0,0,.3)', border: `1px solid ${theme.colors.edge}`, borderRadius: '6px', color: theme.colors.ink, marginBottom: '16px' };
