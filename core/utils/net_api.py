...
class LiveEndpointRegistry:
    ...
    async def refresh(self, force: bool = False) -> None:
        ...
        async with self._lock:
            ...
            self._ts = now
            print(f"[net_api] WARNING: Could not refresh endpoint registry after multiple attempts. Using stale data. Error: {last_exception}")
            # highlight-end
    ...
    def populate_from_app_routes(self, routes: Any):
            ...
            """
            print("[net_api] Populating endpoint registry directly from app object...")
...
def _ensure_mapping(name: str, obj: Optional[Mapping[str, Any]]) -> dict[str, Any]:
    if obj is None: return {}
    if not isinstance(obj, Mapping): raise TypeError(f"{name} must be a Mapping, got {type(obj).__name__}")
    return dict(obj)
...
def _with_internal_defaults(h: Optional[Mapping[str, Any]]) -> dict[str, Any]:
...
async def _request(
    ...
    headers: Optional[Mapping[str, Any]] = None, params: Optional[Mapping[str, Any]] = None,
    ...
    return await client.request(
        method.upper(), _normalize_path(path), json=json,
        headers=hdrs, params=prms, timeout=req_timeout, **kw
    )

async def post(path: str, **kwargs): return await _request("POST", path, **kwargs)
