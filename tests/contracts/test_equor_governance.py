...
async def test_system_action_invokes_full_governance_loop(
    ...
    compose_route = respx_router.post(f"/equor/compose").mock(
        return_value=httpx.Response(200, json=ComposeResponse(
    ...
    # === Assert ===
    assert compose_route.called, "The /equor/compose endpoint was not called for a pre-flight governance check."
